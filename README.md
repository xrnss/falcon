...
4. You may need to modify `club/rnss/launcher/injection/Injector.kt` in the `getTargetPid()` method to accurately detect your target process. 
5. Compile the [Launcher](/Launcher) project by executing the `bootstrapJar` and `launcherJar` Gradle tasks.
6. Execute `Launcher.jar` with the target process open and fully loaded, with the generated `Bootstrap.jar` in the same running directory.
